    ...
    for endpoint_config in "${enabled_endpoints[@]}"; do
        if [[ "$endpoint_config" == *"/v1/chat/completions"* ]]; then
            ...
            if [[ "$remaining_memory_mib" -gt "$completion_memory_needed_mib" ]]; then
                ...
                # unsloth/DeepSeek-R1-Distill-Qwen-14B-bnb-4bit --enforce-eager --enable-reasoning --reasoning-parser deepseek_r1 --quantization bitsandbytes --load-format bitsandbytes --enable-chunked-prefill --max-num-seqs 1 /// this worked @ 16384, and according to https://huggingface.co/spaces/NyxKrage/LLM-Model-VRAM-Calculator using GGUF and Q4_K_M it should support 43008 context size with 14.01GB used total
                start_vllm_server "unsloth/DeepSeek-R1-Distill-Qwen-14B-bnb-4bit" generate "$port" "$completion_memory_fraction_float" "--tokenizer unsloth/DeepSeek-R1-Distill-Qwen-14B --load-format bitsandbytes --quantization bitsandbytes --enable-chunked-prefill --max_model_len 43008 --max-model-len 43008 --max-num-seqs 1 --enforce-eager --enable-reasoning --reasoning-parser deepseek_r1" # --enable-chunked-prefill oddly necessary to avoid OOM issues with this particular model serve setup
